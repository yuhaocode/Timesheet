...
import { approveWeektime, rejectWeektime } from "../Actions/weektime_actions";
...
class TimesheetManagementDetail extends React.Component {
  constructor(props, weekdays) {
    ...
    this.state = {
      ...
      total: 0,
      windex: this.props.match.params.windex
    };
    this.weekdays = [
      "Monday",
      "Tuesday",
  ...
  componentWillMount() {
    if (this.weektime.selectedWeek.length === 0) {
    }
  }
  approve = () => {
    console.log(this.state);
    this.props.approveWeektime(this.state.updateWeektime);
    console.log(1);
    this.props.history.push("/timesheetManagement/");
  };

  reject = () => {
    this.props.rejectWeektime(this.state.updateWeektime);
    console.log("reject");
    this.props.history.push("/timesheetManagement/");
  };

  componentWillMount() {
    let windex = this.props.match.params.windex;
  ...
  render() {
    ...
    if (this.state.redirected) {
      ...
      return <Redirect to="/weektimeManagement" />;
    }
    let { updateWeektime } = this.state;
    ...
    return (
      <div>
        <div style={{ textAlign: "right" }}>
          <Button onClick={this.approve} color="primary">
            Approve
          </Button>
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    approveWeektime: weektime => approveWeektime(weektime, dispatch),
    rejectWeektime: weektime => rejectWeektime(weektime, dispatch)
    // updateInitial: () => updateInitial(dispatch),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(
  withStyles(styles)(TimesheetManagementDetail)
);
